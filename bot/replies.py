...
async def sgpa_enquiry(activity, bot, data):
    query = Query()
    db = TinyDB('./db.json')
    result = db.search(query.userid == activity.from_property.id)
    scraper = Scraper()
    ...
    if len(result) > 0:
        ...
        wak = str(result['wak'])
        scraper.authenticate(rollno, wak)
        sem = data.get_entities()['ORDINAL']
        gpa = scraper.get_gpa(int(sem[0]))
        reply = f"Your SGPA for {sem} semester is " + str(gpa)
        await bot.send_text_activity(activity, reply)
        if gpa > 9.5:
...
async def cgpa_enquiry(activity, bot, data):
    query = Query()
    db = TinyDB('./db.json')
    result = db.search(query.userid == activity.from_property.id)
    scraper = Scraper()
